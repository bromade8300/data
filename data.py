...
SETTINGS = """
{
    "agent_download_url": "https://github.com/wildfoundry/dataplicity-agent/releases/download/v0.5.8/dataplicity",
    "agent_version": "v0.5.8",
...
[program:tuxtunnel]
environment={ENV}
command={PYTHON_PATH} {AGENT_PATH} --server-url {API_URL} run
autorestart=true
...
def to_bytes(data):
    ...
    # Primarily a mechanism to handle Python3 string differences
...
""".format(
    ...
    if not apt_get("supervisor"):
        ...
        check_supervisor()
    if PY3:
        if not apt_get("python3-distutils"):
            log("failed to install python3-distuitls")
...
def get_distro():
    ...
    distro = "unknown"
    try:
        # Not sure if this is any kind of standard
        ...
        log("failed to get distro; {}", error)
        pass
    return distro
...
def download(url, path, urlopen=None):
    ...
    try:
        try:
            url_file = (urlopen or urllib_urlopen)(url)
        ...
        if PY3:
            file_size = int(url_file.info()["Content-Length"])
        else:
...
def register_device(urlopen=None):
    ...
    urlopen = urlopen or urllib_urlopen
    ...
    post_data = urlencode(register_data)
    register_url = settings["register_url"]
    log("posting to {}", register_url)

    try:
        url_file = urlopen(register_url, to_bytes(post_data))
    except URLError:
...
def create_user(getpwnam=None, chown=None, isdir=None):
    ...
    if not settings["dry_run"]:
        ...
        if not isdir(dataplicity_home):
            ...
            chown(dataplicity_home, dataplicity_user.pw_uid, dataplicity_user.pw_gid)

        remote_directory = os.path.join(dataplicity_home, "remote")
        if not isdir(remote_directory):
            log("creating remote dir")
            make_dir(remote_directory)
            chown(remote_directory, dataplicity_user.pw_uid, dataplicity_user.pw_gid)
...
def send_report(report, urlopen=None):
    ...
    urlopen = urlopen or urllib_urlopen
    report_post_body = urlencode(report)
    ...
    url_file = None
    try:
        url_file = urlopen(report_url, to_bytes(report_post_body))
